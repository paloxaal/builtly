...
def _place_karre_from_frontage(skeleton: FieldSkeleton, field: Delfelt, spec: BaseTypologySpec) -> Optional[_PlacementCandidate]:
    ...
    # Ett karrégrep skal leses som ett kvartalsvolum i videre behandling, ikke
    # som fire separate armer som senere konkurrerer om spacing mot hverandre.
    if len(footprints) > 1:
        merged = unary_union(footprints).buffer(0.75, join_style=2).buffer(-0.75, join_style=2).buffer(0)
        merged_parts = [p for p in _flatten_polygons(merged) if p.area > 120.0]
        if merged_parts:
            footprints = [max(merged_parts, key=lambda g: g.area)]
