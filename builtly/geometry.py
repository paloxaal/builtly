...
def _compose_linear_skeleton(core: Polygon, field: Delfelt) -> FieldSkeleton:
    ...
    corridor_count = max(0, int(getattr(field, "view_corridor_count", 0) or 1))
    corridor_w = float(getattr(field, "corridor_width_m", None) or 12.0)
    ...
    if band_h * band_count + total_gap_h > height:
        band_h = max(10.0, (height - total_gap_h) / max(1, band_count))
    y_start = miny + max(0.0, (height - (band_count * band_h + total_gap_h)) / 2.0)

    corridor_polys: List[Polygon] = []
    if corridor_count > 0 and width > corridor_w * corridor_count:
        usable_w = width - corridor_count * corridor_w
        step = usable_w / (corridor_count + 1)
        for i in range(corridor_count):
            ...
            corridor_polys.append(core.intersection(corridor).buffer(0))

    bands: List[Polygon] = []
    row_gaps: List[Polygon] = []
    y = y_start
    for i in range(band_count):
        ...
        for part in _flatten_polygons(band):
            if part.area > 80.0:
                bands.append(part)
        y += band_h
        if i < band_count - 1:
            ...
            if not gap.is_empty and gap.area > 100.0:
                row_gaps.extend(_flatten_polygons(gap))
            y += corridor_w

    courtyard = None
    if row_gaps:
        courtyard = max(row_gaps, key=lambda g: g.area).buffer(0)
    elif corridor_polys:
        courtyard = _largest_polygon(unary_union([c for c in corridor_polys if not c.is_empty]).buffer(0))
    ...
    return FieldSkeleton(field_id=field.field_id, mode="linear_bands", local_orientation_deg=0.0, frontage_lines=frontages, build_bands=bands, courtyard_reserve=courtyard if isinstance(courtyard, Polygon) else _largest_polygon(courtyard), view_corridors=[c for c in corridor_polys if not c.is_empty and c.area > 40.0], accent_nodes=[], open_edges=["south"], frontage_depth_m=frontage_depth, corridor_width_m=corridor_w)
...
def _compose_courtyard_skeleton(core: Polygon, field: Delfelt) -> FieldSkeleton:
    ...
    scale = max(0.45, min(0.75, reserve_ratio ** 0.5))
    ...
    if courtyard is None:
        return FieldSkeleton(field_id=field.field_id, mode="courtyard_frontage", build_bands=[core])
    ...
    open_edges: List[str] = []
    if getattr(field, "design_karre_shape", None) in {"uo", "uo_chamfered"} and getattr(field, "character", "") == "street_facing":
        open_edges = ["south"]
    ...
    if int(getattr(field, "view_corridor_count", 0) or 0) > 0:
        cw = float(getattr(field, "corridor_width_m", None) or 10.0)
        cx = (cminx + cmaxx) / 2.0
        cor = core.intersection(box(cx - cw / 2.0, miny - 1.0, cx + cw / 2.0, maxy + 1.0)).buffer(0)
        if not cor.is_empty and cor.area > 40.0:
            corridors.append(cor)
            bands = [p for g in [b.difference(cor).buffer(0) for b in bands if not b.is_empty] for p in _flatten_polygons(g) if p.area > 80.0]
    ...
    return FieldSkeleton(field_id=field.field_id, mode="courtyard_frontage", local_orientation_deg=0.0, frontage_lines=frontages, build_bands=bands, courtyard_reserve=courtyard, view_corridors=corridors, accent_nodes=[], open_edges=open_edges, frontage_depth_m=float(getattr(field, "frontage_depth_m", None) or 13.0), corridor_width_m=float(getattr(field, "corridor_width_m", None) or 10.0))
...
def _compose_park_skeleton(core: Polygon, field: Delfelt) -> FieldSkeleton:
    ...
    bands = [p for g in [
        core.intersection(box(minx - 1.0, pmaxy, maxx + 1.0, maxy + 1.0)).buffer(0),
        core.intersection(box(minx - 1.0, miny - 1.0, maxx + 1.0, pminy)).buffer(0),
        core.intersection(box(minx - 1.0, pminy, pminx, pmaxy)).buffer(0),
        core.intersection(box(pmaxx, pminy, maxx + 1.0, pmaxy)).buffer(0),
    ] for p in _flatten_polygons(g) if p.area > 80.0]

    nodes: List[Point] = []
    for x, y in [(pminx, pminy), (pminx, pmaxy), (pmaxx, pminy), (pmaxx, pmaxy), ((pminx + pmaxx) / 2.0, pminy), ((pminx + pmaxx) / 2.0, pmaxy)]:
        pt = Point(x, y)
        if core.buffer(-8.0).contains(pt):
            nodes.append(pt)

    corridors: List[Polygon] = []
    cw = float(getattr(field, "corridor_width_m", None) or 14.0)
    if int(getattr(field, "view_corridor_count", 0) or 0) >= 1:
        vc = core.intersection(box((pminx + pmaxx) / 2.0 - cw / 2.0, miny - 1.0, (pminx + pmaxx) / 2.0 + cw / 2.0, maxy + 1.0)).buffer(0)
        if not vc.is_empty and vc.area > 40.0:
            corridors.append(vc)
    if int(getattr(field, "view_corridor_count", 0) or 0) >= 2:
        hc = core.intersection(box(minx - 1.0, (pminy + pmaxy) / 2.0 - cw / 2.0, maxx + 1.0, (pminy + pmaxy) / 2.0 + cw / 2.0)).buffer(0)
        if not hc.is_empty and hc.area > 40.0:
            corridors.append(hc)

    frontages = [f for f in [SkeletonFrontage(role="park_edge", line=_line_from_bounds(core, "north")), SkeletonFrontage(role="park_edge", line=_line_from_bounds(core, "south"))] if f.line is not None]
    return FieldSkeleton(field_id=field.field_id, mode=("park_nodes" if field.typology == Typology.PUNKTHUS else "park_bands"), local_orientation_deg=0.0, frontage_lines=frontages, build_bands=bands, courtyard_reserve=park, view_corridors=corridors, accent_nodes=nodes, open_edges=[], frontage_depth_m=float(getattr(field, "frontage_depth_m", None) or 11.0), corridor_width_m=cw)
...
def _place_lameller_in_bands(skeleton: FieldSkeleton, field: Delfelt, spec: BaseTypologySpec) -> Optional[_PlacementCandidate]:
    if not skeleton.build_bands:
        return None
    variant = getattr(field, "design_variant", None) or "varied"
    footprints: List[Polygon] = []
    for band in sorted(skeleton.build_bands, key=lambda p: (p.centroid.y, p.centroid.x)):
        bminx, bminy, bmaxx, bmaxy = band.bounds
        depth = spec.depth_m.midpoint() if spec.depth_m else 13.0
        if (bmaxy - bminy) < depth * 0.9:
            continue
        y0 = (bminy + bmaxy) / 2.0 - depth / 2.0
        if variant in {"varied", "terraced"}:
            rects = _fit_varied_lameller_in_segment(band, y0, depth, max(spec.length_m.min_m, 30.0), spec.length_m.max_m, min_spacing=6.0)
        else:
            rect = _fit_rect_in_segment(band, y0, depth, spec.length_m.min_m, spec.length_m.max_m)
            rects = [rect] if rect is not None else []
        footprints.extend([r for r in rects if r is not None])
    if len(footprints) < 2 and len(skeleton.build_bands) >= 2:
        return None
    if not footprints:
        return None
    floors_per = None
    ...
    return _evaluate_candidate(footprints, field.target_bra, (field.floors_min, field.floors_max), floors_per_bygg=floors_per)
...
def _place_punkthus_on_nodes(skeleton: FieldSkeleton, field: Delfelt, spec: BaseTypologySpec) -> Optional[_PlacementCandidate]:
    ...
    for size in sizes:
        fps: List[Polygon] = []
        for node in skeleton.accent_nodes:
        ...
        for rect in fps:
            if all(rect.distance(other) >= max(15.0, float(size)) * 0.75 for other in filtered):
...
def _skeleton_summary(skeleton: FieldSkeleton) -> FieldSkeletonSummary:
    return FieldSkeletonSummary(field_id=skeleton.field_id, skeleton_mode=skeleton.mode, frontage_count=len([f for f in skeleton.frontage_lines if f.line is not None]), micro_band_count=len([b for b in skeleton.build_bands if not b.is_empty]), courtyard_reserve_m2=float(getattr(skeleton.courtyard_reserve, "area", 0.0) or 0.0), view_corridor_m2=float(sum(getattr(c, "area", 0.0) for c in skeleton.view_corridors)), accent_node_count=len(skeleton.accent_nodes), build_band_area_m2=float(sum(getattr(b, "area", 0.0) for b in skeleton.build_bands)), frontage_depth_m=float(skeleton.frontage_depth_m), corridor_width_m=float(skeleton.corridor_width_m))
...
def compute_architecture_metrics(plan: Masterplan) -> ArchitectureMetrics:
    ...
    for field in plan.delfelt:
        ...
        frontage = sum(frontage_covs) / max(len(frontage_covs), 1) if frontage_covs else 0.5
        courtyard = 0.5
        if skeleton.courtyard_reserve is not None and skeleton.courtyard_reserve.area > 10.0:
            ...
            occupied_bands = 0
            for band in skeleton.build_bands:
                if union_local is not None and union_local.intersection(band.buffer(1.0)).area > 20.0:
                    occupied_bands += 1
            band_factor = min(1.0, occupied_bands / max(1, len(skeleton.build_bands) - len(skeleton.open_edges)))
            courtyard = 0.5 * free_ratio + 0.5 * band_factor
        view_q = 0.75
        ...
        if buildings:
            ...
            tiny_frac = sum(1 for a in areas if a < mean_area * 0.5) / len(areas)
            entropy = max(0.0, 1.0 - min(1.0, 0.55 * area_cv + 0.45 * tiny_frac))
        ...
        field_total = (0.24 * frontage + 0.22 * courtyard + 0.16 * symmetry + 0.14 * view_q + 0.12 * purity + 0.06 * entropy + 0.06 * bya_fit)
        field_scores.append((field_total, field.polygon.area, frontage, courtyard, symmetry, view_q, entropy, bya_fit))
    ...
    total = max(0.0, min(100.0, 100.0 * (0.24 * frontage + 0.22 * courtyard + 0.16 * symmetry + 0.14 * view_q + 0.12 * typ_purity + 0.06 * entropy + 0.06 * bya_fit)))
    if frontage < 0.55:
    ...
    return ArchitectureMetrics(frontage_continuity=100.0 * frontage, courtyard_clarity=100.0 * courtyard, axis_symmetry=100.0 * symmetry, view_corridor_quality=100.0 * view_q, typology_purity=100.0 * typ_purity, building_entropy=100.0 * entropy, bya_fitness=100.0 * bya_fit, total_score=total, notes=notes)
