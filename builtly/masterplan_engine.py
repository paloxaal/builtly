...
def _should_retry_with_fallback(status_code: int, body_text: str) -> bool:
...
def _call_claude(prompt: str, api_key: str, model: str = DEFAULT_MODEL,
    ...
    Prøver PRIMARY_MODEL først. Ved rate-limit/model-feil prøves FALLBACK_MODEL
    nøyaktig én gang før motoren går videre til deterministisk fallback i resten
    av masterplan-pipelinen.
    """
    ...
    for idx, model_name in enumerate(models):
        try:
            ...
            if idx == 0 and len(models) > 1 and _should_retry_with_fallback(resp.status_code, resp.text):
                logger.warning(
                    "Claude primary model %s failed with %s. Retrying once with fallback model %s.",
                    model_name,
                    resp.status_code,
                    models[1],
                )
            ...
            if idx == 0 and len(models) > 1:
                logger.warning(
                    "Claude primary model %s raised %s. Retrying once with fallback model %s.",
                    model_name,
                    type(exc).__name__,
                    models[1],
                )
