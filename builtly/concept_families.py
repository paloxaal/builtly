...
class LinearMixedStrategy(ConceptStrategy):
    ...
    def envelope_for_field(self, index: int, count: int, field: Optional[Delfelt] = None) -> FieldEnvelope:
        ...
        return FieldEnvelope(
            ...
            design_variant=("terraced" if edge else "varied"),
            ...
            micro_band_count=(3 if edge else 4),
            view_corridor_count=1,
            courtyard_reserve_ratio=(0.10 if edge else 0.12),
            frontage_depth_m=12.0,
            corridor_width_m=10.0,
            macro_structure="spine",
            micro_field_pattern="parallel_bands",
            symmetry_preference="bilateral",
            composition_strictness=0.85,
            frontage_zone_ratio=0.18,
            public_realm_ratio=0.12,
            node_symmetry=True,
        )
...
class ClusterParkStrategy(ConceptStrategy):
    ...
    def envelope_for_field(self, index: int, count: int, field: Optional[Delfelt] = None) -> FieldEnvelope:
        ...
        return FieldEnvelope(
            ...
            frontage_mode=("node" if use_punkthus else "single"),
            micro_band_count=(0 if use_punkthus else 2),
            view_corridor_count=(2 if use_punkthus else 1),
            courtyard_reserve_ratio=0.38,
            frontage_depth_m=11.0,
            corridor_width_m=14.0,
            macro_structure="park_cluster",
            micro_field_pattern=("node_cluster" if use_punkthus else "park_bands"),
            symmetry_preference="radial",
            composition_strictness=0.78,
            frontage_zone_ratio=0.14,
