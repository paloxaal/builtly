import os
import textwrap
from pathlib import Path
from typing import Optional

import streamlit as st

# -------------------------------------------------
# 1) PAGE CONFIG
# -------------------------------------------------
st.set_page_config(
    page_title="Builtly | Engineering Portal",
    page_icon="🏗️",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# -------------------------------------------------
# 2) PAGE MAP
# route = what your deployed Streamlit page responds to
# files = acceptable local file candidates for enabling the link
# -------------------------------------------------
PAGES = {
    "project": {"route": "Project", "files": ["pages/Project.py"]},
    "review": {"route": "Review", "files": ["pages/Review.py"]},
    "geo": {"route": "Geo", "files": ["pages/Geo.py"]},
    "akustikk": {"route": "Akustikk", "files": ["pages/Akustikk.py"]},
    "brann": {"route": "Brannkonsept", "files": ["pages/Brannkonsept.py"]},
    "trafikk": {"route": "Trafikk.py", "files": ["pages/Trafikk.py", "pages/trafikk.py"]},
    "konstruksjon": {"route": "Konstruksjon", "files": ["pages/Konstruksjon.py"]},
    "mulighetsstudie": {"route": "Mulighetsstudie", "files": ["pages/Mulighetsstudie.py"]},
}


# -------------------------------------------------
# 3) HELPERS
# -------------------------------------------------
def page_exists(page_key: str) -> bool:
    config = PAGES.get(page_key, {})
    for candidate in config.get("files", []):
        if Path(candidate).exists():
            return True
    return False


def route_for(page_key: str) -> Optional[str]:
    config = PAGES.get(page_key)
    if not config:
        return None
    # If file exists locally, use configured route.
    # If not, still return route for known pages so staging/live links keep working.
    return config.get("route")


def clean_html(raw: str) -> str:
    """
    Remove common indentation AND left-strip every line so Streamlit Markdown
    never interprets HTML as a code block because of leading spaces.
    """
    dedented = textwrap.dedent(raw)
    return "\n".join(line.lstrip() for line in dedented.splitlines()).strip()


def top_button(page_key: str, label: str, primary: bool = False) -> str:
    route = route_for(page_key)
    classes = "top-btn primary" if primary else "top-btn secondary"
    if route:
        return f'<a href="{route}" target="_self" class="{classes}">{label}</a>'
    return f'<span class="{classes} disabled">{label}</span>'


def hero_button(page_key: str, label: str, primary: bool = False) -> str:
    route = route_for(page_key)
    classes = "hero-btn primary" if primary else "hero-btn secondary"
    if route:
        return f'<a href="{route}" target="_self" class="{classes}">{label}</a>'
    return f'<span class="{classes} disabled">{label}</span>'


def module_card(
    page_key: str,
    icon: str,
    title: str,
    description: str,
    input_text: str,
    output_text: str,
    cta_label: str,
) -> str:
    route = route_for(page_key)
    cta = (
        f'<a href="{route}" target="_self" class="module-cta">{cta_label}</a>'
        if route
        else '<span class="module-cta disabled">In development</span>'
    )
    return clean_html(
        f"""
        <div class="module-card">
            <div class="module-head">
                <div class="module-icon">{icon}</div>
            </div>
            <div class="module-title">{title}</div>
            <div class="module-desc">{description}</div>
            <div class="module-spacer"></div>
            <div class="module-meta">
                <strong>Input:</strong> {input_text}<br/>
                <strong>Output:</strong> {output_text}
            </div>
            <div class="module-cta-wrap">{cta}</div>
        </div>
        """
    )


# -------------------------------------------------
# 4) CSS
# -------------------------------------------------
st.markdown(
    """
<style>
    :root {
        --bg: #06111a;
        --panel: rgba(10, 22, 35, 0.78);
        --panel-2: rgba(13, 27, 42, 0.94);
        --stroke: rgba(120, 145, 170, 0.18);
        --stroke-strong: rgba(120, 145, 170, 0.28);
        --text: #f5f7fb;
        --muted: #9fb0c3;
        --soft: #c8d3df;
        --accent: #38c2c9;
        --accent-2: #78dce1;
        --accent-3: #112c3f;
        --shadow: 0 24px 90px rgba(0,0,0,0.35);
        --radius-xl: 30px;
        --radius-lg: 22px;
        --radius-md: 14px;
    }

    html, body, [class*="css"] {
        font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
    }

    .stApp {
        background:
            radial-gradient(1100px 500px at 15% -5%, rgba(56,194,201,0.18), transparent 50%),
            radial-gradient(900px 500px at 100% 0%, rgba(64,170,255,0.12), transparent 45%),
            linear-gradient(180deg, #071018 0%, #08131d 35%, #071018 100%);
        color: var(--text);
    }

    header[data-testid="stHeader"] {
        visibility: hidden;
        height: 0;
    }

    [data-testid="stSidebar"] {
        background: rgba(7, 16, 24, 0.96);
        border-right: 1px solid var(--stroke);
    }

    .block-container {
        max-width: 1320px !important;
        padding-top: 1.1rem !important;
        padding-bottom: 4rem !important;
    }

    .top-actions {
        display: flex;
        align-items: center;
        justify-content: flex-end;
        gap: 0.75rem;
        flex-wrap: wrap;
    }

    .top-btn {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        min-height: 52px;
        padding: 0.9rem 1.15rem;
        border-radius: 16px;
        text-decoration: none !important;
        font-weight: 700;
        font-size: 0.96rem;
        border: 1px solid transparent;
        transition: all 0.2s ease;
        white-space: nowrap;
        box-shadow: 0 10px 24px rgba(0,0,0,0.18);
    }

    .top-btn.secondary {
        background: rgba(255,255,255,0.045);
        color: var(--text) !important;
        border-color: rgba(120,145,170,0.18);
    }

    .top-btn.secondary:hover {
        background: rgba(255,255,255,0.065);
        border-color: rgba(120,145,170,0.3);
    }

    .top-btn.primary {
        background: linear-gradient(135deg, rgba(77,223,232,0.95), rgba(85,209,228,0.92));
        color: #071018 !important;
        border-color: rgba(120,220,225,0.35);
    }

    .top-btn.primary:hover {
        transform: translateY(-1px);
        box-shadow: 0 14px 28px rgba(56,194,201,0.16);
    }

    .top-btn.disabled,
    .hero-btn.disabled,
    .module-cta.disabled {
        opacity: 0.5;
        pointer-events: none;
    }

    .hero-main,
    .hero-side {
        min-height: 660px;
    }

    .hero {
        position: relative;
        overflow: hidden;
        min-height: 100%;
        background: linear-gradient(180deg, rgba(13,27,42,0.96), rgba(8,18,28,0.96));
        border: 1px solid rgba(120,145,170,0.16);
        border-radius: var(--radius-xl);
        padding: 2.5rem;
        box-shadow: var(--shadow);
    }

    .hero::before {
        content: "";
        position: absolute;
        inset: -80px -120px auto auto;
        width: 420px;
        height: 420px;
        background: radial-gradient(circle, rgba(56,194,201,0.18) 0%, transparent 62%);
        pointer-events: none;
    }

    .eyebrow {
        color: var(--accent-2);
        text-transform: uppercase;
        letter-spacing: 0.16em;
        font-size: 0.8rem;
        font-weight: 800;
        margin-bottom: 1.1rem;
    }

    .hero-title {
        font-size: clamp(2.8rem, 5.6vw, 5rem);
        line-height: 0.98;
        letter-spacing: -0.05em;
        font-weight: 850;
        margin: 0;
        color: var(--text);
        max-width: 10.5ch;
    }

    .hero-title .accent {
        color: var(--accent-2);
    }

    .hero-subtitle {
        margin-top: 1.4rem;
        max-width: 58ch;
        font-size: 1.12rem;
        line-height: 1.86;
        color: var(--soft);
    }

    .hero-note {
        margin-top: 1.05rem;
        max-width: 58ch;
        font-size: 0.98rem;
        line-height: 1.75;
        color: var(--muted);
    }

    .hero-actions {
        display: flex;
        align-items: center;
        gap: 0.8rem;
        flex-wrap: wrap;
        margin-top: 1.7rem;
    }

    .hero-btn {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        min-height: 52px;
        padding: 0.88rem 1.2rem;
        border-radius: 16px;
        text-decoration: none !important;
        font-weight: 750;
        font-size: 0.98rem;
        transition: all 0.2s ease;
        border: 1px solid transparent;
        white-space: nowrap;
    }

    .hero-btn.primary {
        background: linear-gradient(135deg, rgba(77,223,232,0.95), rgba(85,209,228,0.92));
        color: #071018 !important;
        border-color: rgba(120,220,225,0.35);
    }

    .hero-btn.primary:hover {
        transform: translateY(-1px);
        box-shadow: 0 14px 28px rgba(56,194,201,0.16);
    }

    .hero-btn.secondary {
        background: rgba(255,255,255,0.04);
        color: var(--text) !important;
        border-color: rgba(120,145,170,0.18);
    }

    .hero-btn.secondary:hover {
        background: rgba(255,255,255,0.06);
        border-color: rgba(120,145,170,0.3);
    }

    .proof-strip {
        display: flex;
        align-items: center;
        flex-wrap: wrap;
        gap: 0.65rem;
        margin-top: 1.25rem;
    }

    .proof-chip {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        padding: 0.46rem 0.75rem;
        border-radius: 999px;
        border: 1px solid rgba(120,145,170,0.16);
        background: rgba(255,255,255,0.03);
        color: var(--soft);
        font-size: 0.84rem;
        white-space: nowrap;
    }

    .hero-panel {
        min-height: 100%;
        background: rgba(255,255,255,0.03);
        border: 1px solid var(--stroke);
        border-radius: 28px;
        padding: 1.35rem;
        display: flex;
        flex-direction: column;
    }

    .panel-title {
        font-size: 0.9rem;
        text-transform: uppercase;
        letter-spacing: 0.08em;
        color: var(--muted);
        margin-bottom: 1rem;
        font-weight: 700;
    }

    .mini-stat {
        background: rgba(255,255,255,0.03);
        border: 1px solid var(--stroke);
        border-radius: 18px;
        padding: 1.15rem 1.2rem;
        margin-bottom: 0.85rem;
        flex: 1;
    }

    .mini-stat:last-child {
        margin-bottom: 0;
    }

    .mini-stat-value {
        font-size: 1.45rem;
        font-weight: 800;
        color: var(--text);
        line-height: 1.1;
    }

    .mini-stat-label {
        margin-top: 0.35rem;
        color: var(--muted);
        font-size: 0.92rem;
        line-height: 1.6;
    }

    .section-head {
        margin-top: 3rem;
        margin-bottom: 1.1rem;
    }

    .section-kicker {
        color: var(--accent-2);
        text-transform: uppercase;
        letter-spacing: 0.14em;
        font-size: 0.76rem;
        font-weight: 800;
        margin-bottom: 0.5rem;
    }

    .section-title {
        font-size: clamp(2rem, 4.1vw, 3.15rem);
        font-weight: 820;
        letter-spacing: -0.04em;
        color: var(--text);
        margin: 0;
        line-height: 1.04;
    }

    .section-subtitle {
        margin-top: 0.65rem;
        color: var(--muted);
        line-height: 1.82;
        max-width: 72ch;
        font-size: 1.03rem;
    }

    .loop-grid {
        display: grid;
        grid-template-columns: repeat(4, minmax(0, 1fr));
        gap: 1rem;
        margin-top: 1rem;
    }

    .loop-card {
        background: var(--panel-2);
        border: 1px solid var(--stroke);
        border-radius: 20px;
        padding: 1.1rem;
        min-height: 220px;
    }

    .loop-number {
        width: 40px;
        height: 40px;
        display: inline-flex;
        align-items: center;
        justify-content: center;
        border-radius: 999px;
        background: rgba(56,194,201,0.12);
        border: 1px solid rgba(56,194,201,0.22);
        color: var(--accent-2);
        font-weight: 800;
        font-size: 0.95rem;
        margin-bottom: 0.95rem;
    }

    .loop-title {
        font-size: 1.02rem;
        font-weight: 720;
        color: var(--text);
        margin-bottom: 0.55rem;
    }

    .loop-desc {
        font-size: 0.96rem;
        line-height: 1.72;
        color: var(--muted);
    }

    .module-grid {
        display: grid;
        grid-template-columns: repeat(3, minmax(0, 1fr));
        gap: 1.25rem;
        align-items: stretch;
        margin-top: 1.2rem;
    }

    .module-card {
        background: linear-gradient(180deg, rgba(12,25,39,0.98), rgba(8,18,28,0.98));
        border: 1px solid var(--stroke);
        border-radius: 24px;
        padding: 1.25rem;
        box-shadow: 0 12px 38px rgba(0,0,0,0.18);
        display: flex;
        flex-direction: column;
        min-height: 390px;
        height: 100%;
        transition: all 0.2s ease;
    }

    .module-card:hover {
        border-color: rgba(56,194,201,0.22);
        box-shadow: 0 16px 44px rgba(0,0,0,0.22);
        transform: translateY(-2px);
    }

    .module-head {
        display: flex;
        align-items: center;
        justify-content: flex-start;
        margin-bottom: 1rem;
    }

    .module-icon {
        width: 52px;
        height: 52px;
        border-radius: 16px;
        display: inline-flex;
        align-items: center;
        justify-content: center;
        background: rgba(56,194,201,0.1);
        border: 1px solid rgba(56,194,201,0.18);
        color: var(--accent-2);
        font-size: 1.45rem;
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.04);
    }

    .module-title {
        font-size: 1.14rem;
        font-weight: 780;
        color: var(--text);
        margin-bottom: 0.65rem;
        letter-spacing: -0.02em;
        line-height: 1.35;
    }

    .module-desc {
        font-size: 0.98rem;
        line-height: 1.75;
        color: var(--muted);
    }

    .module-spacer {
        flex: 1 1 auto;
        min-height: 1rem;
    }

    .module-meta {
        font-size: 0.9rem;
        line-height: 1.8;
        color: var(--soft);
        padding-top: 0.95rem;
        border-top: 1px solid rgba(120,145,170,0.14);
    }

    .module-cta-wrap {
        margin-top: 1rem;
    }

    .module-cta {
        width: 100%;
        display: inline-flex;
        align-items: center;
        justify-content: center;
        min-height: 46px;
        padding: 0.78rem 0.95rem;
        border-radius: 14px;
        text-decoration: none !important;
        font-weight: 760;
        font-size: 0.94rem;
        border: 1px solid rgba(56,194,201,0.26);
        background: rgba(56,194,201,0.08);
        color: var(--accent-2) !important;
        transition: all 0.2s ease;
        box-sizing: border-box;
    }

    .module-cta:hover {
        background: rgba(56,194,201,0.14);
        border-color: rgba(56,194,201,0.42);
        color: #c9fbff !important;
    }

    .cta-band {
        margin-top: 3rem;
        margin-bottom: 1.5rem;
        background: linear-gradient(135deg, rgba(56,194,201,0.12), rgba(18,49,76,0.28));
        border: 1px solid rgba(56,194,201,0.18);
        border-radius: 26px;
        padding: 1.55rem;
        box-shadow: 0 16px 42px rgba(0,0,0,0.16);
    }

    .cta-title {
        font-size: 1.42rem;
        font-weight: 820;
        color: var(--text);
        margin-bottom: 0.35rem;
        letter-spacing: -0.02em;
    }

    .cta-desc {
        color: var(--muted);
        line-height: 1.78;
        max-width: 72ch;
        font-size: 1rem;
    }

    .footer-block {
        text-align: center;
        margin-top: 2.6rem;
        padding-top: 2rem;
        border-top: 1px solid rgba(120,145,170,0.14);
    }

    .footer-name {
        color: var(--text);
        font-weight: 760;
        font-size: 1rem;
        margin-bottom: 0.25rem;
    }

    .footer-meta {
        color: rgba(159,176,195,0.6);
        font-size: 0.84rem;
    }

    @media (max-width: 1180px) {
        .module-grid {
            grid-template-columns: repeat(2, minmax(0, 1fr));
        }

        .loop-grid {
            grid-template-columns: repeat(2, minmax(0, 1fr));
        }

        .hero-main,
        .hero-side {
            min-height: unset;
        }
    }

    @media (max-width: 760px) {
        .module-grid,
        .loop-grid {
            grid-template-columns: 1fr;
        }

        .hero-title {
            max-width: none;
        }

        .top-actions {
            justify-content: flex-start;
        }
    }
</style>
""",
    unsafe_allow_html=True,
)

# -------------------------------------------------
# 5) TOP BAR
# -------------------------------------------------
left, right = st.columns([0.72, 0.28], gap="small")
with left:
    if os.path.exists("logo-white.png"):
        st.image("logo-white.png", width=300)
    elif os.path.exists("logo.png"):
        st.image("logo.png", width=300)
    else:
        st.markdown("<h2 style='margin:0;color:white;'>Builtly</h2>", unsafe_allow_html=True)

with right:
    st.markdown(
        clean_html(
            f"""
            <div class="top-actions">
                {top_button('project', 'Project Setup', primary=False)}
                {top_button('review', 'QA & Sign-off', primary=True)}
            </div>
            """
        ),
        unsafe_allow_html=True,
    )

# -------------------------------------------------
# 6) HERO
# -------------------------------------------------
hero_left, hero_right = st.columns([1.38, 0.78], gap="large")

with hero_left:
    st.markdown(
        clean_html(
            f"""
            <div class="hero hero-main">
                <div class="eyebrow">The Builtly Loop</div>
                <h1 class="hero-title">From <span class="accent">raw data</span> to signed deliverables.</h1>
                <div class="hero-subtitle">
                    Builtly is the customer portal for compliance-grade engineering delivery. Upload project inputs,
                    let the platform validate, calculate, apply rule checks, and draft the deliverable - before junior QA
                    and senior sign-off turn it into a consistent, traceable, submission-ready package.
                </div>
                <div class="hero-note">
                    Built for building applications, execution, and professional compliance - designed as a production workflow,
                    not a showcase UI.
                </div>
                <div class="hero-actions">
                    {hero_button('project', 'Open project setup', primary=True)}
                    {hero_button('review', 'Open QA and sign-off', primary=False)}
                </div>
                <div class="proof-strip">
                    <div class="proof-chip">Rules-first</div>
                    <div class="proof-chip">Audit trail</div>
                    <div class="proof-chip">PDF + DOCX output</div>
                    <div class="proof-chip">Digital sign-off</div>
                    <div class="proof-chip">Structured QA workflow</div>
                </div>
            </div>
            """
        ),
        unsafe_allow_html=True,
    )

with hero_right:
    st.markdown(
        clean_html(
            """
            <div class="hero-panel hero-side">
                <div class="panel-title">Why Builtly?</div>
                <div class="mini-stat">
                    <div class="mini-stat-value">80-90%</div>
                    <div class="mini-stat-label">Reduction in manual drafting and repetitive report production</div>
                </div>
                <div class="mini-stat">
                    <div class="mini-stat-value">Junior + Senior</div>
                    <div class="mini-stat-label">Human-in-the-loop QA, technical control, and digital sign-off</div>
                </div>
                <div class="mini-stat">
                    <div class="mini-stat-value">PDF + DOCX</div>
                    <div class="mini-stat-label">Complete report packages with appendices and traceability</div>
                </div>
                <div class="mini-stat">
                    <div class="mini-stat-value">Full Traceability</div>
                    <div class="mini-stat-label">Inputs, versions, compliance checks, and signatures logged end-to-end</div>
                </div>
            </div>
            """
        ),
        unsafe_allow_html=True,
    )

# -------------------------------------------------
# 7) WORKFLOW
# -------------------------------------------------
st.markdown(
    clean_html(
        """
        <div class="section-head">
            <div class="section-kicker">Workflow</div>
            <h2 class="section-title">The Builtly Loop</h2>
            <div class="section-subtitle">
                A deterministic four-step production flow that takes project data from upload to checked, reviewable documentation.
            </div>
        </div>
        <div class="loop-grid">
            <div class="loop-card">
                <div class="loop-number">1</div>
                <div class="loop-title">Input</div>
                <div class="loop-desc">Upload PDFs, IFC models, XLSX lab files, drawings, and project-specific data in one place.</div>
            </div>
            <div class="loop-card">
                <div class="loop-number">2</div>
                <div class="loop-title">Validate and analyze</div>
                <div class="loop-desc">The platform parses, validates, applies local rule checks, performs calculations, and drafts the deliverable.</div>
            </div>
            <div class="loop-card">
                <div class="loop-number">3</div>
                <div class="loop-title">QA and sign-off</div>
                <div class="loop-desc">Junior review, senior technical assessment, and digital sign-off - with version control throughout.</div>
            </div>
            <div class="loop-card">
                <div class="loop-number">4</div>
                <div class="loop-title">Output</div>
                <div class="loop-desc">Finalized documentation package in standard formats, ready for municipal submission or execution.</div>
            </div>
        </div>
        """
    ),
    unsafe_allow_html=True,
)

# -------------------------------------------------
# 8) MODULES
# -------------------------------------------------
module_cards = [
    module_card(
        "geo",
        "🌍",
        "GEO / ENV - Ground Conditions",
        "Analyze lab files and excavation plans. Classifies masses, proposes disposal logic, and drafts environmental action plans.",
        "XLSX / CSV / PDF + plans",
        "Environmental action plan, logs",
        "Open Geo & Env",
    ),
    module_card(
        "akustikk",
        "🔊",
        "ACOUSTICS - Noise & Sound",
        "Ingest noise maps and floor plans. Generates facade requirements, window specifications, and mitigation strategies.",
        "Noise map + floor plan",
        "Acoustics report, facade evaluation",
        "Open Acoustics",
    ),
    module_card(
        "brann",
        "🔥",
        "FIRE - Safety Strategy",
        "Evaluate architectural drawings against building codes. Generates escape routes, fire cell division, and strategy.",
        "Architectural drawings + class",
        "Fire strategy concept, deviations",
        "Open Fire Strategy",
    ),
    module_card(
        "trafikk",
        "🚦",
        "TRAFFIC - Mobility",
        "Traffic generation, parking requirements, access logic, and mobility planning for project delivery and early-phase decisions.",
        "Site plans, local norms",
        "Traffic memo, mobility plan",
        "Open Traffic & Mobility",
    ),
    module_card(
        "konstruksjon",
        "🏢",
        "STRUC - Structural Concept",
        "Conceptual structural checks, principle dimensioning, and integration with carbon footprint estimations.",
        "Models, load parameters",
        "Concept memo, grid layouts",
        "Open Structural",
    ),
    module_card(
        "mulighetsstudie",
        "📐",
        "ARK - Feasibility Study",
        "Site screening, volume analysis, and early-stage decision support before full engineering design.",
        "Site data, zoning plans",
        "Feasibility report, utilization metrics",
        "Open Feasibility",
    ),
]

st.markdown(
    clean_html(
        f"""
        <div class="section-head">
            <div class="section-kicker">Engineering disciplines</div>
            <h2 class="section-title">Specialized agents in one platform</h2>
            <div class="section-subtitle">
                Each discipline has dedicated ingestion logic, discipline-specific checks, and output templates while sharing the same portal, validation, QA, and sign-off backbone.
            </div>
        </div>
        <div class="module-grid">{''.join(module_cards)}</div>
        """
    ),
    unsafe_allow_html=True,
)

# -------------------------------------------------
# 9) CTA BAND
# -------------------------------------------------
st.markdown(
    clean_html(
        f"""
        <div class="cta-band">
            <div class="cta-title">Not just analysis. Actual deliverables.</div>
            <div class="cta-desc">
                Builtly operates as a full-stack delivery system: create a project, upload raw data, review deviations,
                generate drafts, execute QA, and download the signed documentation package.
            </div>
            <div class="hero-actions" style="margin-top:1rem;">
                {hero_button('project', 'Start in project setup', primary=True)}
                {hero_button('review', 'Go to QA and sign-off', primary=False)}
            </div>
        </div>
        """
    ),
    unsafe_allow_html=True,
)

# -------------------------------------------------
# 10) FOOTER
# -------------------------------------------------
st.markdown(
    clean_html(
        """
        <div class="footer-block">
            <div class="footer-name">Builtly AS</div>
            <div class="footer-meta">© 2026 Builtly. All rights reserved.</div>
        </div>
        """
    ),
    unsafe_allow_html=True,
)
