...
def create_full_report_pdf(name, client, content, maps):
    ...
    toc = [
        ...
        "4. ATKOMST OG MYKE TRAFIKANTER", 
        "5. PARKERING (BIL OG SYKKEL)", 
        "6. VARELEVERING OG RENOVASJON", 
...
with st.expander("3. Visuelt Grunnlag (Situasjonsplan / Adkomst)", expanded=True):
    st.info("Viktig: Last opp utomhusplan, situasjonsplan eller kart over nabolaget. Agenten vil analysere hvor det er mest hensiktsmessig å legge atkomst, plassere sykkelparkering og håndtere renovasjon.")
    files = st.file_uploader("Last opp kart/planer (PDF/Bilder)", accept_multiple_files=True, type=['png', 'jpg', 'jpeg', 'pdf'])
...
if st.button("🚀 Kjør Trafikkanalyse (RITra)", type="primary", use_container_width=True):
    ...
    if files:
        with st.spinner("📐 Henter ut kart og situasjonsplaner for visuell AI-analyse..."):
            try:
                for f in files: 
    ...
    with st.spinner(f"🤖 Analyserer mobilitet og genererer trafikknotat for {pd_state.get('land', 'Norge')}..."):
        ...
        prompt_text = f"""
        Du er Builtly RITra AI, en senior trafikkplanlegger og mobilitetsrådgiver.
        Skriv et formelt "Trafikknotat og Mobilitetsplan" for prosjektet:
        ...
        REGELVERK: {pd_state.get('land', 'Norge (TEK17 / Veinormaler)')}. 
        
        KUNDENS PROSJEKTBESKRIVELSE: 
        "{pd_state['p_desc']}"
        
        VISUELL ANALYSE AV VEDLAGTE SITUASJONSPLANER/KART:
        Jeg har lagt ved bilder. Din oppgave er å NØYE analysere utomhus-situasjonen:
        1. Vurder hvor atkomst for bil bør legges for å unngå konflikt med fotgjengere.
        2. Identifiser hvor renovasjonsbil (søppelbil) og varelevering kan rygge og snu.
        3. Se etter muligheter for etablering av trygg sykkelparkering.
        
        INSTRUKSER (Skriv formelt, teknisk og presist, min 1200 ord):
        - Flett inn dine spesifikke observasjoner fra bildene i teksten.
        - Beregn en teoretisk trafikkgenerering (ÅDT) basert på bygningens areal ({bta} m2) og funksjon ({b_type}).
        - Anbefal konkrete tiltak for myke trafikanter (sykkel/gange) basert på at kollektivdekningen er oppgitt til: {kollektivdekning}.
        
        STRUKTUR (Bruk KUN disse nøyaktige overskriftene):
        # 1. SAMMENDRAG OG KONKLUSJON
        # 2. PROSJEKTBESKRIVELSE OG REGELVERK
        # 3. TRAFIKKGENERERING OG KAPASITET (Inkluder estimat på antall kjøretøy pr døgn)
        # 4. ATKOMST OG MYKE TRAFIKANTER (Beskriv observasjoner fra situasjonsplan)
        # 5. PARKERING (BIL OG SYKKEL) (Tilpass etter {p_norm})
        # 6. VARELEVERING OG RENOVASJON (Vurder snuplass og tilkomst)
        """
        ...
        try:
            ...
            with st.spinner("Kompilerer RITra-PDF med vedlagte kart/planer..."):
                pdf_data = create_full_report_pdf(p_name, c_name, res.text, images_for_ai)
            st.success("✅ Trafikknotat er ferdigstilt!")
            st.download_button("📄 Last ned Trafikknotat", pdf_data, f"Builtly_RITra_{p_name}.pdf", type="primary")
        except Exception as e: 
