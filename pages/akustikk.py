...
def get_model():
    """Direkte tilkobling for Streamlit Cloud (Bypasser list_models som gir 404)"""
...
def create_full_report_pdf(name, client, content, maps):
    ...
    toc = [
        ...
        "4. BEREGNINGSFORUTSETNINGER OG METODIKK", "5. RESULTATER: STØYUTBREDELSE", 
        "6. VURDERING AV FASADEISOLERING OG TILTAK", "7. VURDERING AV UTEOPPHOLDSAREALER", "VEDLEGG: STØYSONEKART"
    ]
            ...
            try:
                ...
                for i in range(process_limit):
                    ...
                    if f.name.lower().endswith('pdf'):
                        ...
                        # MINNEOPTIAMALISERING: Redusert oppløsning for å unngå at skyen krasjer
                        doc = fitz.open(stream=f.read(), filetype="pdf")
                        pix = doc.load_page(0).get_pixmap(matrix=fitz.Matrix(1.5, 1.5))
                        img = Image.open(io.BytesIO(pix.tobytes("png")))
                        doc.close() # Frigjør minne umiddelbart
                    else:
                        img = Image.open(f)
                    ...
                    # FRIGJØR MINNE ETTER HVER ETASJE
                    img.close()
                    gc.collect()
