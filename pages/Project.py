...
<style>
    ...
    [data-testid="stPageLink-NavLink"]:hover {
        background-color: rgba(56,194,201,0.15); border-color: rgba(56,194,201,0.8); transform: translateY(-2px);
    }
...
with input_col:
    ...
    with tab1:
        ...
        # Språk/Land Selector (Styrer AI-ens språk og regelsjekker)
        ...
        new_land = st.selectbox(
            "🌍 Land / Lokalt Regelverk (Styrer AI-språk og regelverk)", 
            land_options, 
            index=l_idx,
            help="Dette valget forteller AI-modulene hvilket språk rapporten skal skrives på, og hvilke nasjonale byggeregler (f.eks. TEK17 vs BBR) som gjelder for prosjektet."
        )
        ...
        new_p_desc = st.text_area(
            ...
            value=pd["p_desc"],
            height=140,
            placeholder="Beskriv prosjektet kort her. Dette gir AI-en kontekst til å skrive langt bedre rapporter. (f.eks. 'Oppføring av 4-etasjers kontorbygg med underjordisk parkering...')"
        )

    with tab2:
        st.markdown("<br>", unsafe_allow_html=True)
        if "Norge" in new_land:
            st.info("💡 **Kartverket API:** Skriv inn adresse *eller* Gnr/Bnr og trykk på knappen under for å autoutfylle resten.")
    ...
    if st.button("💾 Lagre & Synkroniser SSOT Data", type="primary", use_container_width=True):
        ...
        st.success(f"✅ Data lagret! Prosjektet '{new_p_name}' er nå synkronisert.")
        st.rerun()
...
# --- 8. LAUNCHPAD (Neste steg - dukker opp når data er lagret/fylt ut) ---
if completeness > 30:
