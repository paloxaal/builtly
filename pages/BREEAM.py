...
# --- 1. TEKNISK OPPSETT ---
st.set_page_config(page_title="BREEAM Assistant | Builtly", layout="wide", initial_sidebar_state="collapsed")
...
if st.session_state.project_data.get("p_name") in ["", "Nytt Prosjekt"]:
    ...
    st.info("BREEAM-agenten trenger kontekst om prosjektet for å generere en prosjektspesifikk pre-analyse.")
...
class BuiltlyProPDF(FPDF):
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 10, clean_pdf_text(f"PROSJEKT: {self.p_name} | Dokumentnr: BREEAM-001"), 0, 1, 'R')
    ...
    def footer(self):
        ...
        self.cell(0, 10, clean_pdf_text(f'PRE-ASSESSMENT UTKAST | Side {self.page_no()}'), 0, 0, 'C')
...
def create_full_report_pdf(name, client, content, maps):
    ...
    pdf.multi_cell(0, 12, clean_pdf_text('BREEAM PRE-ASSESSMENT'), 0, 'L')
    pdf.ln(2)
    ...
    pdf.draw_cover_meta_items([("OPPDRAGSGIVER:", client), ("DATO:", datetime.now().strftime("%d. %m. %Y")), ("UTARBEIDET AV:", "Builtly BREEAM AI Engine"), ("STATUS:", "Foreløpig / Pre-analyse")])
    ...
    toc = ['1. ANALYSEMETADATA', '2. SERTIFISERINGSFORUTSETNINGER', '3. AMBISJONSBILDE', '4. KREDITT- / EMNEVURDERING', '5. EVIDENSOVERSIKT', '6. GAP-ANALYSE', '7. HANDLINGSLAN NESTE 30/60/90 DAGER', '8. SPØRSMÅL TIL AP ELLER REVISOR']
    for t in toc:
        pdf.set_x(25)
    ...
    pdf.add_page()
    render_report_body(pdf, content, field_labels=['Status', 'Vurdering', 'Manglende evidens', 'Anbefaling', 'Ansvarlig', 'Problemstilling', 'Tiltak', 'Frist', 'Gjelder'], metadata_sections=['1. ANALYSEMETADATA', '2. SERTIFISERINGSFORUTSETNINGER'])
    ...
    if maps and len(maps) > 0:
        ...
        for i, m in enumerate(maps):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.jpg') as tmp:
                ...
                pdf.cell(0, 10, clean_pdf_text(f"Figur V-{i+1}: Dokument visuelt vurdert for BREEAM-kreditter av AI-agenten."), 0, 1, 'C')
...
# --- 6. STREAMLIT UI ---
st.markdown(f"<h1 style='font-size: 2.5rem; margin-bottom: 0;'>🌿 BREEAM Assistant</h1>", unsafe_allow_html=True)
st.markdown("<p style='color: var(--muted); font-size: 1.1rem; margin-bottom: 2rem;'>AI-agent for BREEAM-NOR pre-analyse, gap-analyse og evidensmatrise.</p>", unsafe_allow_html=True)
...
with st.expander("2. BREEAM-NOR Forutsetninger", expanded=True):
    st.info("Angi overordnede mål. AI-en vil generere en realistisk handlingsplan basert på dette ambisjonsnivået.")
    c3, c4 = st.columns(2)
    ambisjonsniva = c3.selectbox("Målsatt Sertifiseringsnivå", ["Pass", "Good", "Very Good", "Excellent", "Outstanding"], index=2)
    manual_versjon = c4.selectbox("Manualversjon", ["BREEAM-NOR v6.0", "BREEAM-NOR v2016", "Uavklart / Pre-registrering"], index=0)
    sertifiseringsloype = st.selectbox("Sertifiseringsløype", ["Nybygg", "Rehabilitering", "Beskpoke / Tilpasset"], index=0)

with st.expander("3. Visuelt Grunnlag & Underlag", expanded=True):
    st.info("BREEAM-agenten vurderer underlaget for å se om grunnleggende poeng (som økologi og transport) er dokumenterbare.")
    ...
    if len(saved_images) > 0:
        st.success(f"📎 Fant {len(saved_images)} felles arkitekttegninger fra Project Setup. Disse vurderes automatisk for BREEAM-potensial!")
    else:
        st.warning("Ingen felles tegninger funnet. Du bør laste opp situasjonsplan / arkitektur under.")
        
    files = st.file_uploader("Last opp notater, skisser eller miljørapporter (PDF/Bilder)", accept_multiple_files=True, type=['png', 'jpg', 'jpeg', 'pdf'])

st.markdown("<br>", unsafe_allow_html=True)

if st.button("🚀 Kjør BREEAM Pre-assessment", type="primary", use_container_width=True):
    ...
    st.info(f"Klar! Sender totalt {len(images_for_ai)} filer til BREEAM-agenten for vurdering.")
                
    with st.spinner(f"🤖 Kjører gap-analyse mot manual {manual_versjon}..."):
        try:
        ...
        - Målsatt nivå: {ambisjonsniva}
        - Manualversjon: {manual_versjon}
        - Sertifiseringsløype: {sertifiseringsloype}
        ...
        3. BREEAM-koder som MAN (Management) og WST (Waste) er uforståelige for vanlige folk. Du MÅ skrive fulle norske navn i overskriftene. Eksempel: Bruk "### Ledelse og styring (MAN 01)" i stedet for bare "### MAN 01".
        ...
        # 4. KREDITT- / EMNEVURDERING (Bruk ### for emnene med FULLE NORSKE NAVN, og nøkkelordene Status:, Vurdering: og Manglende evidens: under hver av dem)
        ...
        try:
            ...
            with st.spinner("Kompilerer BREEAM-PDF med corporate design..."):
                ...
                doc_id = f"PRJ-{datetime.now().strftime('%y')}-BREEAM{st.session_state.review_counter:03d}"
                st.session_state.review_counter += 1
                
                st.session_state.pending_reviews[doc_id] = {
                    "title": pd_state.get('p_name', 'Nytt Prosjekt'),
                    "module": "BREEAM Assistant",
                    "drafter": "Builtly AI",
                    "reviewer": "BREEAM AP / Revisor",
                    "status": "Pending AP Review",
                    "class": "badge-phase2",
                    "pdf_bytes": pdf_data
                }

            st.session_state.generated_breeam_pdf = pdf_data
            st.session_state.generated_breeam_filename = f"Builtly_BREEAM_{p_name}.pdf"
            st.rerun() 
...
if "generated_breeam_pdf" in st.session_state:
    st.success("✅ BREEAM Pre-assessment er ferdigstilt og sendt til QA-køen for gjennomgang av AP!")
    
    col_dl, col_qa = st.columns(2)
    with col_dl:
        st.download_button("📄 Last ned Pre-assessment", st.session_state.generated_breeam_pdf, st.session_state.generated_breeam_filename, type="primary", use_container_width=True)
    with col_qa:
        if find_page("Review"):
