...
# --- 1. TEKNISK OPPSETT ---
st.set_page_config(page_title="Akustikk & Støy (RIAku) | Builtly", layout="wide", initial_sidebar_state="collapsed")
...
def create_full_report_pdf(name, client, content, maps):
    ...
    pdf.cell(0, 15, clean_pdf_text("AKUSTISK KONSEPT OG STØYVURDERING"), 0, 1, 'L')
    pdf.set_font('Helvetica', '', 16); pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 10, clean_pdf_text(f"FORPROSJEKT: {pdf.p_name}"), 0, 1, 'L'); pdf.ln(30)
    
    for l, v in [("OPPDRAGSGIVER:", client), ("DATO:", datetime.now().strftime("%d. %m. %Y")), ("UTARBEIDET AV:", "Builtly RIAku AI Engine"), ("REGELVERK:", pd_state.get('land', 'Norge'))]:
    ...
    toc = [
        ...
        "3. UTENDØRS STØYKILDER OG STØYSONER", 
        "4. KRAV TIL FASADEISOLERING", 
        "5. INNENDØRS LYDFORHOLD (ETTERKLANG & TRINNLYD)", 
        "6. VIBRASJON OG TEKNISKE INSTALLASJONER", 
        "VEDLEGG: VURDERT TEGNINGS- OG STØYGRUNNLAG"
    ]
    for t in toc:
    ...
    if maps:
        ...
        for i, m in enumerate(maps):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp:
                ...
                pdf.cell(0, 10, clean_pdf_text(f"Figur V-{i+1}: Dokument visuelt analysert av RIAku-agenten."), 0, 1, 'C')

    return bytes(pdf.output(dest='S'))

# --- STREAMLIT UI ---
st.markdown(f"<h1 style='font-size: 2.5rem; margin-bottom: 0;'>🔊 RIAku — Akustikk & Støy</h1>", unsafe_allow_html=True)
st.markdown("<p style='color: var(--muted); font-size: 1.1rem; margin-bottom: 2rem;'>AI-agent for vurdering av fasadeisolering, utendørs støykrav og innendørs lydforhold.</p>", unsafe_allow_html=True)
...
with st.expander("1. Prosjekt & Lokasjon (Auto-synced)", expanded=True):
    ...
    st.info(f"📍 **Regelverk:** Agenten vil bruke **{pd_state.get('land', 'Norge')}** og gjeldende standarder (f.eks. NS 8175 i Norge) som juridisk utgangspunkt.")

with st.expander("2. Akustiske Parametere & Støykilder", expanded=True):
    c3, c4 = st.columns(2)
    støysone = c3.selectbox("Definert Støysone (Utendørs)", ["Hvit/Grønn sone (Tilstrekkelig stille)", "Gul sone (Krever tiltak)", "Rød sone (Som hovedregel ikke tillatt for støyfølsomt formål)"], index=1)
    hovedkilde = c4.selectbox("Hovedkilde til støy", ["Veitrafikk", "Jernbane / Sporvei", "Flystøy", "Industri / Næring", "Blandet / Annet"])

with st.expander("3. Visuelt Grunnlag (Støykart & Fasader)", expanded=True):
    st.info("Viktig: Last opp støykart fra kartlegging, samt fasadetegninger eller plantegninger. Agenten vil vurdere hvilke fasader som er mest utsatt, og identifisere stille sider.")
    files = st.file_uploader("Last opp Støykart, Fasader eller Plantegninger (PDF/Bilder)", accept_multiple_files=True, type=['png', 'jpg', 'jpeg', 'pdf'])

st.markdown("<br>", unsafe_allow_html=True)
if st.button("🚀 Kjør Akustisk Vurdering (RIAku)", type="primary", use_container_width=True):
    
    images_for_ai = [] 
    
    if files:
        with st.spinner("📐 Henter ut bilder for visuell AI-analyse av støysoner og fasader..."):
            try:
                for f in files: 
    ...
    with st.spinner(f"🤖 Analyserer tegninger og genererer akustikkrapport etter {pd_state.get('land', 'Norge')}..."):
        try:
        ...
        prompt_text = f"""
        Du er Builtly RIAku AI, en senior akustikkrådgiver.
        Skriv et formelt "Akustisk konsept / Støyvurdering" for prosjektet:
        
        PROSJEKT: {p_name} ({b_type}, {pd_state['bta']} m2, {pd_state['etasjer']} etasjer). 
        LOKASJON: {adresse}.
        STØYSONE: {støysone}.
        HOVEDKILDE TIL STØY: {hovedkilde}.
        
        REGELVERK: {pd_state.get('land', 'Norge (TEK17)')}. Referer til relevante nasjonale standarder (f.eks. NS 8175 hvis Norge, SS 25268 hvis Sverige).
        
        KUNDENS PROSJEKTBESKRIVELSE: 
        "{pd_state['p_desc']}"
        
        VISUELL ANALYSE AV VEDLAGTE DOKUMENTER (Støykart/Tegninger):
        Jeg har lagt ved bilder. Din oppgave er å analysere disse:
        1. Identifiser hvilke fasader som vender mot støykilden (eksponert side).
        2. Identifiser om det finnes en "stille side" (skjermet fasade) hvor det bør etableres uteplasser eller plasseres soverom.
        3. Vurder behovet for oppgraderte vinduer (lydglass) basert på byggets utforming.
        ...
        - Gi konkrete råd for fasadeisolering og planløsning (hvor bør soverom ligge?).
        - Ta for deg innendørs akustikk (trinnlyd mellom etasjer, etterklangstid i fellesarealer/trapp).
        
        STRUKTUR (Bruk KUN disse nøyaktige overskriftene):
        # 1. SAMMENDRAG OG KONKLUSJON
        # 2. PROSJEKTBESKRIVELSE OG REGELVERK
        # 3. UTENDØRS STØYKILDER OG STØYSONER (Beskriv ut fra støysone og kart)
        # 4. KRAV TIL FASADEISOLERING (Beskriv tiltak og vinduskrav for eksponerte fasader)
        # 5. INNENDØRS LYDFORHOLD (ETTERKLANG & TRINNLYD)
        # 6. VIBRASJON OG TEKNISKE INSTALLASJONER
        """
        ...
        try:
            ...
            with st.spinner("Kompilerer RIAku-PDF med vedlagte dokumenter..."):
                pdf_data = create_full_report_pdf(p_name, c_name, res.text, images_for_ai)
            st.success("✅ RIAku Rapport er ferdigstilt!")
            st.download_button("📄 Last ned Akustikk & Støy-rapport", pdf_data, f"Builtly_RIAku_{p_name}.pdf", type="primary")
        except Exception as e: 
